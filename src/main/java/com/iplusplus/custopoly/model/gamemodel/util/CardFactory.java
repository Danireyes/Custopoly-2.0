...
public abstract class CardFactory {
    ...
    private static final String CARD_ENVELOPE= "Envelope";
    private static final String CARD_BLACK = "BlackCard";
    ...
    public static ArrayList<Card> readEnvelopeCards(InputStream file) {
        return readCards(file, CARD_ENVELOPE);
    }

    public static ArrayList<Card> readBlackCardCards (InputStream file) {
        return readCards(file, CARD_BLACK);
    }
		...
		while(scanner.hasNextLine()) {
            ...
            if (!chanceInfo.isEmpty()) {
                ...
                if (getCardType(cardData).equals(CARD_CHANCE) && cardType.equals(CARD_CHANCE)) {
                    ...
                    addCard(cards, cardData);
                } else if (getCardType(cardData).equals(CARD_ENVELOPE) && cardType.equals(CARD_ENVELOPE)) {
                    addCard(cards, cardData);
                } else if (getCardType(cardData).equals(CARD_BLACK) && cardType.equals(CARD_BLACK)) {
                    addCard(cards, cardData);
                }
	...
	private static void addCard(ArrayList<Card> cards, ArrayList<String> cardData) {
		Card card = new Card(getCardText(cardData), getCardCommand(cardData), getCardType(cardData));
		cards.add(card);
	}
	...
	private static Command getCardCommand(ArrayList<String> cardData) {
		...
		if (commandTitle.equals(COMMAND_GETPRIZE)) {
			...
			return new PayFeeForCardCommand(Integer.parseInt(getValueAt(cardData, 3)));
