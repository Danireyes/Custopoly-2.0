...
public class GameTheme implements Serializable {
    ...
    private ArrayList<Integer> cellsResources;

    private static final int NUMBEROFCELLS = 40;
	...
	public GameTheme(String name, double price, HashSet<PlayerSkin> playerSkinsList) {
        ...
        initializeCellsResources();
	}

    public Integer getCellResource(int index) {
        return cellsResources.get(index);
    }
    ...
    private void initializeCellsResources() {
        this.cellsResources = new ArrayList<>();
        for (int i = 0; i < NUMBEROFCELLS; i++){
            String resourceName = this.getName() + "_square_" + i;
            Integer resource = Utilities.getResId(resourceName, R.drawable.class);
            this.cellsResources.add(resource);
        }
    }
}
